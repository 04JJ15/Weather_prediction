...
│   ├── train_optuna.py     # LightGBM, catboost, RandomForest 학습 및 저장
│   └── ensemble.py         # 학습한 모델로 앙상블(스태킹) 진행 
...
# 공모전 모델 성능 비교 (10단계 → 11-3.4 앙상블)

---

## 1️⃣ 10단계 (베이스라인: 단일 LightGBM)

| Target               | MAE    | RMSE   | 가중치 | 가중 점수 |
|----------------------|-------:|-------:|-------:|----------:|
| 습도(%)_관측         | 5.9097 | 7.7887 | 0.3    | 2.0548    |
| 기온(degC)_관측      | 0.9673 | 1.2514 | 0.5    | 0.5547    |
| 대기압(mmHg)_관측    | 0.3666 | 0.4777 | 0.2    | 0.0844    |
| **최종 스코어**      |        |        |        | **2.6939**|

> **설명:**  
> 기본 피처만 사용한 LightGBM 단일 모델로 최종 가중 평균 RMSE **2.6939** 달성

---

## 2️⃣ 11-1단계 (시간 파생 변수 추가)

| Target               | MAE    | RMSE   | (MAE+RMSE)/2 | 가중치 | 가중 점수 |
|----------------------|-------:|-------:|-------------:|-------:|----------:|
| 습도(%)_관측         | 4.7764 | 6.3366 |       5.5565 | 0.3    | 1.6669    |
| 기온(degC)_관측      | 0.8152 | 1.0564 |       0.9358 | 0.5    | 0.4679    |
| 대기압(mmHg)_관측    | 0.3055 | 0.4007 |       0.3531 | 0.2    | 0.0706    |
| **최종 스코어**      |        |        |             |        | **2.2055**|

> **효과:**  
> `hour`, `month`, `weekday` 등의 시간 정보를 추가하여  
> **2.6939 → 2.2055** (↓0.4884) 로 대폭 개선

---

## 3️⃣ 11-2단계 (교호작용 피처 추가)

| Target               | MAE    | RMSE   | (MAE+RMSE)/2 | 가중치 | 가중 점수 |
|----------------------|-------:|-------:|-------------:|-------:|----------:|
| 습도(%)_관측         | 4.7195 | 6.2527 |       5.4861 | 0.3    | 1.6458    |
| 기온(degC)_관측      | 0.8111 | 1.0495 |       0.9303 | 0.5    | 0.4652    |
| 대기압(mmHg)_관측    | 0.3025 | 0.3974 |       0.3499 | 0.2    | 0.0700    |
| **최종 스코어**      |        |        |             |        | **2.1810**|

> **효과:**  
> `일사량×기온`, `습도×기온` 등 상호작용 피처 추가로 **2.2055 → 2.1810** (↓0.0245) 소폭 개선

---

## 4️⃣ 11-3단계 (하이퍼파라미터 튜닝 & CV)

### 4-1. GridSearchCV + 3-fold TimeSeriesSplit
- **습도 CV RMSE:** 11.2447  
- **전체 데이터:** MAE 6.3167 / RMSE 8.1586  

### 4-2. GridSearchCV + 5-fold TimeSeriesSplit
- **습도 CV RMSE:** 11.3202  
- **전체 데이터 RMSE:** 8.8745  

### 4-3. Optuna + 5-fold TimeSeriesSplit
...
#설명:
#CV 도입 후 일반화 오차(≈11.2) 확인 → Optuna 튜닝으로 소폭 개선
```
## 11-3.4단계 (앙상블 + 최종 평가)

| Target               | MAE    | RMSE   | 가중 점수 |
|----------------------|-------:|-------:|----------:|
| 습도(%)_관측         | 3.9540 | 5.2227 | 1.3765    |
| 기온(degC)_관측      | 0.3142 | 0.4183 | 0.1831    |
| 대기압(mmHg)_관측    | 0.2833 | 0.3749 | 0.0658    |
| **최종 스코어**      |        |        | **1.6254**|

**설명:**
- 세 개의 최적화된 베이스 모델(LightGBM / CatBoost / RandomForest)의 예측값을  
  메타 모델(RidgeCV)로 결합하는 **스태킹 파이프라인**을 적용  
- 최종 가중 평균 RMSE **1.6254** → 이전 단계 대비 크게 개선

---

✏️ **핵심 요약**
1. **10단계 → 11-1:**  
   - 시간 파생 변수 추가 → **2.6939 → 2.2055** (↓ 0.4884)  
2. **11-1 → 11-2:**  
   - 교호작용 피처 추가 → **2.2055 → 2.1810** (↓ 0.0245)  
3. **11-2 → 11-3.1/3.2:**  
   - CV 도입 시 일반화 오차 확인 (습도 CV RMSE ≈ 11.2)  
4. **11-3.3:**  
   - Optuna 튜닝으로 CV RMSE 소폭 개선 (습도 11.32 → 11.15)  
5. **11-3.4 (앙상블):**  
   - 메타 모델 결합 후 최종 평가 → **1.6254** 달성
